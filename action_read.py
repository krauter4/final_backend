...
argv.add_argument("-m", "--model", default = 'resnet-34_kinetics.onnx', help="specify path to pre-trained model")
argv.add_argument("-c", "--classes", default = 'Actions.txt', help="specify path to class labels file")
...
while True:
    ...
    # Check if the label is "tasting beer"
    if label == "tasting beer":
        text_color = (0, 0, 255)  # Bright red color for warning
    else:
        text_color = (255, 255, 255)  # Default white color for other activities

    # Draw label on each frame
    for frame in originals:
        # Append predicted activity
        cv2.rectangle(frame, (0, 0), (300, 40), (0, 0, 0), -1)
        cv2.putText(frame, label, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, text_color, 2)
